...
// Handlers
export async function handleCreateAgent(params: { agentId: string; preset?: string }) {
...
export async function handleExecuteSkill(params: { agentId: string; skillName: string }) {
  ...
  if (!skill) {
    ...
    throw new Error(`Skill "${validated.skillName}" não encontrada. Disponíveis: ${availableSkills.join(', ')}`);
  }
