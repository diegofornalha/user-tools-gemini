...
export async function createEkyteAgent(
  config: Partial<import('./types.js').EkyteNavigatorConfig> = {},
) {
...
export class SessionAnalyzer {
  static analyzeSession(session: import('./types.js').EkyteSession) {
    ...
    return {
      ...
      productivity:
        session.skillsLearned.length /
        Math.max(session.skillsAttempted.length, 1),
      quality:
        1 - session.errors.length / Math.max(session.observations.length, 1),
    };
  }
  ...
  static generateSessionSummary(sessions: import('./types.js').EkyteSession[]) {
    ...
    return {
      ...
      averageSessionDuration:
        analyses.reduce((sum, a) => sum + a.duration, 0) / sessions.length,
      totalSkillsLearned: analyses.reduce((sum, a) => sum + a.skillsLearned, 0),
      averageSuccessRate:
        analyses.reduce((sum, a) => sum + a.successRate, 0) / sessions.length,
      averageAutonomyLevel:
        analyses.reduce((sum, a) => sum + a.autonomyLevel, 0) / sessions.length,
      totalScreenshots: analyses.reduce(
        (sum, a) => sum + a.screenshotsTaken,
        0,
      ),
      mostProductiveSession: analyses.reduce((max, a) =>
        a.productivity > max.productivity ? a : max,
      ),
      bestQualitySession: analyses.reduce((max, a) =>
        a.quality > max.quality ? a : max,
      ),
    };
  }
}
